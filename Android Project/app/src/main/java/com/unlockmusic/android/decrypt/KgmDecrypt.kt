package com.unlockmusic.android.decrypt

import com.unlockmusic.android.util.AudioUtils

object KgmDecrypt {

    private val VPR_HEADER = byteArrayOf(
        0x05, 0x28, 0xBC.toByte(), 0x96.toByte(), 0xE9.toByte(), 0xE4.toByte(), 0x5A, 0x43,
        0x91.toByte(), 0xAA.toByte(), 0xBD.toByte(), 0xD0.toByte(), 0x7A, 0xF5.toByte(), 0x36, 0x31
    )

    private val KGM_HEADER = byteArrayOf(
        0x7C, 0xD5.toByte(), 0x32, 0xEB.toByte(), 0x86.toByte(), 0x02, 0x7F, 0x4B,
        0xA8.toByte(), 0xAF.toByte(), 0xA6.toByte(), 0x8E.toByte(), 0x0F, 0xFF.toByte(), 0x99.toByte(), 0x14
    )

    private val VPR_MASK_DIFF = byteArrayOf(
        0x25, 0xDF.toByte(), 0xE8.toByte(), 0xA6.toByte(), 0x75, 0x1E, 0x75, 0x0E,
        0x2F, 0x80.toByte(), 0xF3.toByte(), 0x2D.toByte(), 0xB8.toByte(), 0xB6.toByte(), 0xE3.toByte(), 0x11, 0x00
    )

    private val MASK_V2_PRE_DEF = intArrayOf(
        
    )

    private val TABLE1 = intArrayOf(
        
    )

    private val TABLE2 = intArrayOf(
        
    )

    private fun getMask(pos: Long): Int {
        var offset = pos shr 4
        var value = 0
        while (offset >= 0x11) {
            value = value xor TABLE1[(offset % 272).toInt()]
            offset = offset shr 4
            value = value xor TABLE2[(offset % 272).toInt()]
            offset = offset shr 4
        }
        return MASK_V2_PRE_DEF[(pos % 272).toInt()] xor value
    }

    fun decrypt(fileData: ByteArray, filename: String): DecryptionResult {
        try {
            val ext = AudioUtils.getFileExtension(filename).lowercase()
            val isVpr = ext == "vpr"

            val expectedHeader = if (isVpr) VPR_HEADER else KGM_HEADER
            if (!AudioUtils.hasPrefix(fileData, expectedHeader)) {
                return DecryptionResult.fail("不是有效的 KGM/VPR 文件")
            }

            val headerLen = AudioUtils.readInt32LE(fileData, 0x10)

            // Read key: 16 bytes from offset 0x1C, plus 0 at position 16
            val key = ByteArray(17)
            System.arraycopy(fileData, 0x1C, key, 0, 16)
            key[16] = 0

            val audioData = fileData.copyOfRange(headerLen, fileData.size)

            for (i in audioData.indices) {
                val pos = i
                var med8 = (key[pos % 17].toInt() and 0xFF) xor (audioData[i].toInt() and 0xFF)
                med8 = med8 xor ((med8 and 0xF) shl 4)

                var msk8 = getMask(pos.toLong())
                msk8 = msk8 xor ((msk8 and 0xF) shl 4)

                audioData[i] = (med8 xor msk8).toByte()

                if (isVpr) {
                    audioData[i] = (audioData[i].toInt() xor VPR_MASK_DIFF[pos % 17].toInt()).toByte()
                }
            }

            val outExt = AudioUtils.detectAudioExtension(audioData)
            val outName = AudioUtils.removeExtension(filename) + ".$outExt"

            return DecryptionResult.ok(audioData, outName, outExt)
        } catch (e: Exception) {
            return DecryptionResult.fail("KGM解密失败: ${e.message}")
        }
    }
}
